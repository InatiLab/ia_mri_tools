...
from ia_mri_tools.features import input_normalization, riff, NUM_SCALES, NORMALIZATION_SCALE
...
@click.command()
@click.option('--output', type=click.STRING, default='out.nii', help='Output filename for the corrected image.')
@click.option('--scale', type=click.INT, default=NORMALIZATION_SCALE, help='Scale for high pass filtering and smoothing.')
...
@click.argument('input_image', type=click.STRING)
@click.option('--nscales', type=click.INT, default=NUM_SCALES, help='number of spatial scales')
@click.option('--normalization_scale', type=click.INT, default=NORMALIZATION_SCALE, help='scale for output gain control')
def estimate_textures(input_image, nscales, normalization_scale, output):
    ...
    t, names = riff(im.get_data(), nscales=nscales, normalization_scale=normalization_scale)
    nfeats = len(t)
